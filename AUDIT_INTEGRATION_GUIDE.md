...
// Al eliminar usuario
await logAudit(req.userId, "delete_user", "user", id, {
...
```javascript
// Al crear producto
await logAudit(req.userId, "create_product", "product", newProduct.id, {
...
await logAudit(req.userId, "update_product", "product", id, {
  fields_updated: Object.keys(updateData),
});

// Al eliminar producto
await logAudit(req.userId, "delete_product", "product", id, {
...
await logAudit(req.userId, "harvest_stored", "inventory", productId, {
  quantity: quantity,
  source: "finca",
...
```javascript
// Al enviar a tostar
await logAudit(req.userId, "roast_sent", "lot", lotId, {
  lot_code: lot.code,
  weight: weight,
  roaster: roasterName,
});
...
await logAudit(req.userId, "roast_received", "lot", lotId, {
  lot_code: lot.code,
