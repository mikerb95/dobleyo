...
- **`docs/CASOS_DE_USO.md`** — iteración P2/P3: 10 casos de uso nuevos (CU-028..CU-037: detalle de producto, historial de pedidos, perfil, contacto, dashboard de producción, presupuestos, facturación, blog, auditoría, exportaciones) y 7 RF P2 integrados como pasos/flujos de CUs existentes (RF-005→CU-001, RF-014→CU-002, RF-026→CU-003, RF-056→CU-007, RF-083→CU-018, RF-092→CU-019, RF-116→CU-025). Matriz actualizada: cobertura total de RFs funcionales; quedan como brecha documentada HU-008 y HU-036 sin RF asociado (CU-031 y CU-036 lo señalan) y RF-106/107 como transversales de UI.
- **`src/pages/admin/devtools.astro`** — nueva pestaña "Diagrama UML" junto a "Zona de peligro" (patrón de tabs `.adm-tabs`/`.adm-panel` tomado de `/admin/finanzas`). Renderiza en el navegador el diagrama de casos de uso de `docs/CASOS_DE_USO.md` usando Mermaid (`mermaid.min.js` cargado desde `cdn.jsdelivr.net`, ya permitido en el `scriptSrc` del CSP de `server/index.js`/`api/index.js`) y permite descargar el resultado como SVG. Renderizado perezoso (solo al abrir la pestaña) para no cargar la librería si el usuario no la usa.
- **`docs/CASOS_DE_USO.md` + `src/pages/admin/devtools.astro`** — revisión de corrección UML: la flecha «extend» entre CU-003 y CU-007 estaba invertida (en UML el extend apunta del caso que extiende hacia el caso base); corregida a `CU-007 -.«extend».-> CU-003` en el diagrama del doc y en el visor de devtools. Se reescribió la lista de relaciones con la convención extensión-primero y se aclaró en la leyenda que las flechas punteadas sin estereotipo representan precedencia de la cadena de trazabilidad, no include/extend.
