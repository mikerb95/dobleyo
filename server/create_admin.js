...
async function createAdmin() {
  ...
  try {
    ...
    if (existing.rows.length > 0) {
      ...
      await db.query("UPDATE users SET password_hash = ?, role = ?, is_verified = 1 WHERE email = ?", [hash, 'admin', ADMIN_EMAIL]);
      console.log('Admin password and role updated.');
    } else {
      const hash = await auth.hashPassword(ADMIN_PASS);
      await db.query(
        'INSERT INTO users (email, password_hash, first_name, role, is_verified) VALUES (?, ?, ?, ?, ?)',
        [ADMIN_EMAIL, hash, 'Admin DobleYo', 'admin', 1]
      );
